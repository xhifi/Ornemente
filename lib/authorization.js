...
export const hasPermission = async (scope, resource) => {
  const session = await auth.api.getSession({ headers: await headers() });
  if (!session) return false;

  const roles = session?.user?.roles.map((role) => role.role_name);
  if (roles.some((role) => role === "banned")) return false;

  const permissions = session?.user?.permissions || [];

  // Support both new resource-specific format (products.read) and old format
  const permissionName = `${resource}.${scope}`;

  // Check if user has the specific resource-scoped permission
  const hasSpecificPermission = permissions.some((permission) => permission?.permission_name === permissionName);

  // Fallback: Check old format for backward compatibility
  const hasLegacyPermission = permissions.some(
    (permission) => permission?.permission_name === scope && permission?.resource_name === resource
  );

  return hasSpecificPermission || hasLegacyPermission;
};

/*
  permissions = [
    {scopes: ["read", "write", "update"], resource: "users"},
    {scopes: ["read", "write"], resource: "products"},
  ]
*/
export const hasAllPermissions = async (permissions) => {
  ...
  const destructedPermissions =
    permissions.reduce((acc, p) => {
      p.scopes.forEach((scope) => {
        acc.push({ scope, resource: p.resource });
      });
      return acc;
    }, []) || [];

  return destructedPermissions.every((permission) => {
    const permissionName = `${permission.resource}.${permission.scope}`;

    // Check new resource-specific format
    const hasSpecificPermission = userPermissions.some((userPermission) => userPermission?.permission_name === permissionName);

    // Fallback: Check old format for backward compatibility
    const hasLegacyPermission = userPermissions.some(
      (userPermission) => userPermission?.permission_name === permission.scope && userPermission?.resource_name === permission.resource
    );
...
export const hasAnyPermission = async (permissions) => {
  ...
  const userPermissions =
    session?.user?.permissions.map((permission) => ({
      scope: permission.permission_name,
  ...
  return destructedPermissions.some((permission) =>
    userPermissions.some((userPermission) => userPermission.scope === permission.scope && userPermission.resource === permission.resource)
  );
};
