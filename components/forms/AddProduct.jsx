...
import DropTarget from "@/components/ui/factory/form/drop-target";
import { z } from "zod";
...
const defaultValues = {
  ...
  pieces: [],
  images: [],
  colors: 1, // Set a default value for colors
};

const AddProduct = ({ sexes, types, brands, sizes, designs, fabrics, colors, action }) => {
  ...
  const [productSizes, setProductSizes] = useState({}); // Store size data {code: {selected: bool, stock: number}}
  const [selectedDesigns, setSelectedDesigns] = useState([]);
  const [productPieces, setProductPieces] = useState([]); // Store product pieces/parts data
  ...
  // Effect to initialize product sizes with default values
  useEffect(() => {
    if (sizes && sizes.data) {
      const initialSizes = {};
  ...
  // Effect to sync images state with the form
  useEffect(() => {
  ...
  const handleCreateDesign = (designName) => {
    ...
    // 1. Call an API to create the design
    // 2. Get the new design object back with an ID
    // 3. Add it to the selectedDesigns array
  };
  ...
  if (errors.pieces) {
    console.log("Pieces errors:", errors.pieces);
  }
  ...
  return (
    <>
      <form className="px-6 py-12 space-y-3" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          ...
          <div className={`flex flex-wrap gap-2 ${errors.sex ? "border-red-500" : ""}`}>
            <Controller
              name="sex"
              control={control}
              defaultValue={sexes.data.find((g) => g.name === "women").id}
              render={({ field }) => (
                <>
        ...
        {/* This designs select is now provided in a more comprehensive version further down the form */}
