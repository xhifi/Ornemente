    ...
    <div className="flex items-center gap-3 border p-1.5 w-1/2 md:w-auto">
      {product.selected_quantity > 1 ? (
        <button
          ...
          onClick={() => updateQuantity(product.sku, product.selected_quantity - 1)}
        >
          -
        </button>
      ) : (
      ...
      <button
        ...
        disabled={product.selected_quantity >= product.available_stock}
        onClick={() => {
          if (product.selected_quantity < product.available_stock) {
            return updateQuantity(product.sku, product.selected_quantity + 1);
          }
        }}
      >
        +
